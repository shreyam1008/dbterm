...
func TestBuildCommandPaletteItemsIncludesObjectsRecentQueriesAndEffectiveShortcut(t *testing.T) {
	...
	app := &App{
		...
		databaseObjects: map[int]databaseObjectListItem{
			2: {objType: utils.ObjViews, name: "active_users"},
			3: {objType: utils.ObjFunctions, name: "normalize_email"},
			4: {objType: utils.ObjStoredProcedures, name: "rebuild_totals"},
			5: {objType: utils.ObjTriggers, name: "audit_orders"},
			6: {objType: utils.ObjExtensions, name: "uuid-ossp"},
		},
	}
